 ...
 * All calls go through the PHP backend at /api/
 */

const API_BASE = (import.meta.env.VITE_API_URL || '/api').replace(/\/$/, '');

export const apiConfigured = !!import.meta.env.VITE_API_URL || true; // always true when self-hosted
...
class Query<T> {
  ...
  // .in() is used for batch acknowledge — handled via special endpoint; stub here
  in(_col: string, _vals: string[]) { return this; }

  ilike(col: string, pattern: string) {
    // Search param: strip leading/trailing %
    this.params['search'] = pattern.replace(/%/g, '');
    return this;
  }

  or(filter: string) {
    // Extract search term from "col.ilike.%term%" patterns
...
class Table<T> {
  ...
  update(fields: Partial<T>) {
    return {
      ...
      in: async (_col: string, vals: string[]): Promise<{ error: string | null }> => {
        // Batch update via special endpoint
  ...
  async upsert(
    ...
    // Use system-settings upsert endpoint for settings, otherwise batch insert
    ...
    // For other tables, POST each record
